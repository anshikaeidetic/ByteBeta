...
from byte.utils import lazy_optional_module, load_optional_module
...
class ViT(BaseEmbedding):
    ...
    Example:
        .. code-block:: python
            ...
            from byte.embedding import ImageEmbedding
    ...
    def __init__(self, model: str = "google/vit-base-patch16-384") -> None:
        load_optional_module("torchvision", package="torchvision")
        self.model_name = model
        model = transformers.ViTModel.from_pretrained(model)
        self.model = model.eval()
        config = self.model.config
        self.__dimension = config.hidden_size

    def to_embeddings(self, data, **__) -> Any:
    ...
    def preprocess(self, data) -> Any:
        image_processor = transformers.AutoImageProcessor.from_pretrained(self.model_name)
