...
paddle = lazy_optional_module("paddle", package="paddlepaddle")
paddlenlp_transformers = lazy_optional_module(
...
class PaddleNLP(BaseEmbedding):
    ...
    def __init__(self, model: str = "ernie-3.0-medium-zh") -> None:
        self.model = paddlenlp_transformers.AutoModel.from_pretrained(model)
        self.model.eval()

        self.tokenizer = paddlenlp_transformers.AutoTokenizer.from_pretrained(model)
    ...
    def to_embeddings(self, data, **_) -> Any:
        ...
        if not isinstance(data, list):
            data = [data]
        inputs = self.tokenizer(data, padding=True, truncation=True, return_tensors="pd")
    ...
    def post_proc(self, token_embeddings, inputs) -> Any:
        ...
        sentence_embs = paddle.sum(token_embeddings * input_mask_expanded, 1) / paddle.clip(
