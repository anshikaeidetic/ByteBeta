"""ByteAI Cache adapter for DeepSeek chat models.

DeepSeek exposes an OpenAI-compatible chat API. This adapter routes
DeepSeek requests through ByteAI Cache's shared runtime while normalizing
responses to the same OpenAI-compatible shape used by the other adapters.
...
Set ``DEEPSEEK_API_KEY`` in your environment or pass ``api_key`` directly.
Optionally override the base URL with ``DEEPSEEK_API_BASE`` or ``api_base``.
"""
...
def _require_api_key(api_key=None) -> Any:
    resolved = api_key or os.getenv("DEEPSEEK_API_KEY")
    if not resolved:
        raise ValueError("Missing DeepSeek credentials. Provide api_key or set DEEPSEEK_API_KEY.")
    return resolved


def _api_url(api_base=None) -> str:
    base = str(api_base or os.getenv("DEEPSEEK_API_BASE") or _DEFAULT_API_BASE).strip().rstrip("/")
...
def _response_to_openai(payload: dict[str, Any], requested_model: str) -> dict[str, Any]:
    ...
    for index, choice in enumerate(choices):
        ...
        if message.get("tool_calls"):
            normalized_message["tool_calls"] = list(message.get("tool_calls") or [])
...
def _stream_chunks(response) -> Iterator[dict[str, Any]]:
    yield from iter_openai_sse_chunks(response, provider="deepseek")
        ...
        def cache_data_convert(cache_data) -> Any:
            if kwargs.get("stream", False):
                return replay_as_stream(
                    str(cache_data), model=str(kwargs.get("model", "") or "deepseek-chat")
                )
            return cls._cache_data_convert(cache_data)

        kwargs = cls.fill_base_args(**kwargs)
        kwargs = apply_native_prompt_cache(
            "deepseek", kwargs, kwargs.get("cache_obj", cache).config
        )
        return adapt(
            cls._llm_handler,
            cache_data_convert,
