...
def summarize_to_length(summarizer, text, target_len, max_len=1024) -> Any:
    tokenizer = summarizer.tokenizer

    def token_length(text) -> Any:
...
class SummarizationContextProcess(ContextProcess):
    ...
    def __init__(self, model_name="facebook/bart-large-cnn", tokenizer=None, target_length=512) -> None:
    ...
    def summarize_to_sentence(self, sentences, target_size=1000) -> Any:
        lengths = []
        for sentence in sentences:
            lengths.append(len(sentence))
        total_length = np.array(lengths).sum()
        target_lengths = [int(target_size * val_len / total_length) for val_len in lengths]
        target_sentences = []
