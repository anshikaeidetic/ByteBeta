...
from byte.utils import import_pymilvus
...
def _ensure_pymilvus() -> None:
    global connections, utility, FieldSchema, DataType
    global CollectionSchema, Collection, MilvusException
    ...
    import_pymilvus()
    try:
        from pymilvus import (  # pylint: disable=C0415
        ...
        from pymilvus import (
            MilvusException as _MilvusException,
        )
        from pymilvus import (
        ...
        raise ModuleNotFoundError(
            "pymilvus is unavailable or incompatible; install a compatible pymilvus/protobuf stack"
        ) from exc
...
class Milvus(VectorBase):
    ...
    def __init__(
        ...
        self.search_params = search_params or self.SEARCH_PARAM[self.index_params["index_type"]]

    def _create_local(self, port, local_data) -> None:
        from byte.utils import import_milvus_lite  # pylint: disable=import-outside-toplevel
    ...
    def _connect(self, host, port, user, password, secure) -> None:
        try:
            i = [
                connections.get_connection_addr(x[0]) for x in connections.list_connections()
    ...
    def _create_collection(self, collection_name) -> None:
        if not utility.has_collection(collection_name, using=self.alias):
            ...
            byte_log.warning(
                "The %s collection already exists, and it will be used directly.", collection_name
            )
            self.col = Collection(collection_name, consistency_level="Strong", using=self.alias)

        if len(self.col.indexes) == 0:
            try:
                byte_log.info("Attempting creation of Milvus index.")
                self.col.create_index("embedding", index_params=self.index_params)
                byte_log.info("Creation of Milvus index successful.")
            except MilvusException as e:
                byte_log.warning(
                    "Error with building index: %s, and attempting creation of default index.", e
                )
