...
from byte.utils.error import NotFoundError, ParamError
...
PGVECTOR_URL = "postgresql://postgres@localhost:5432/postgres"
...
COLLECTION_NAME = "byte"

WEAVIATE_TIMEOUT_CONFIG = (10, 60)
WEAVIATE_STARTUP_PERIOD = 5

_FAISS_FALLBACK_WARNED = False


def _fallback_faiss_vector_base(index_path: str, dimension: int, top_k: int) -> Any:
    """Use the hnswlib implementation, which already has a pure-Python fallback."""
    global _FAISS_FALLBACK_WARNED
    from byte.manager.vector_data.hnswlib_store import Hnswlib

    if not _FAISS_FALLBACK_WARNED:
        byte_log.warning(
            "faiss-cpu is unavailable; falling back to the built-in hnswlib-compatible vector store."
        )
        _FAISS_FALLBACK_WARNED = True
    vector_base = Hnswlib(
    ...
    vector_base._byte_requested_store = "faiss"
    vector_base._byte_fallback_store = "hnswlib"
    return vector_base
...
class VectorBase:
    ...
    Generate specific VectorBase with the configuration. For example, setting for
       ...
       `distributed` (with `stores` or `shards`, `top_k`, and `shard_strategy` params).

    :param name: the name of the vectorbase, it supports 'milvus', 'faiss', 'chromadb', 'hnswlib',
        'pgvector', 'docarray', 'usearch', 'redis', 'qdrant', 'weaviate', and 'distributed'.
    :type name: str
    ...
    :type port: str
    :param user: the user for Zilliz Cloud, defaults to None.
    :type user: str | None
    :param password: the password for Zilliz Cloud, defaults to None.
    ...
    :param collection_name: the name of the collection for Milvus vector database, defaults to 'byte'.
    ...
    :type index_params: dict
    :param collection_name: the prefix of the table for PostgreSQL pgvector, defaults to 'byte'.
    :type collection_name: str
    ...
    @staticmethod
    def check_dimension(dimension) -> None:
        if dimension <= 0:
            raise ParamError(
    ...
    @staticmethod
    def get(name, **kwargs) -> Any:
        top_k = kwargs.get("top_k", TOP_K)
        if name == "milvus":
            from byte.manager.vector_data.milvus import Milvus
            ...
            dimension = kwargs.get("dimension", DIMENSION)
            index_path = kwargs.pop("index_path", kwargs.pop("index_file_path", FAISS_INDEX_PATH))
            VectorBase.check_dimension(dimension)
            try:
                ...
                vector_base = _fallback_faiss_vector_base(index_path, dimension, top_k)
        elif name == "chromadb":
            from byte.manager.vector_data.chroma import Chromadb
            ...
            dimension = kwargs.get("dimension", DIMENSION)
            index_path = kwargs.pop(
                "index_path", kwargs.pop("index_file_path", "./hnswlib_index.bin")
            )
            ...
            from byte.manager.vector_data.pgvector import PGVector
            ...
            index_path = kwargs.pop(
                "index_path", kwargs.pop("index_file_path", "./docarray_index.bin")
            )
            vector_base = DocArrayIndex(index_file_path=index_path, top_k=top_k)
        elif name == "usearch":
            from byte.manager.vector_data.usearch import USearch

            dimension = kwargs.get("dimension", DIMENSION)
            index_path = kwargs.pop("index_path", kwargs.pop("index_file_path", "./index.usearch"))
            ...
            from byte.manager.vector_data.redis_vectorstore import RedisVectorStore
            ...
            from byte.manager.vector_data.qdrant import QdrantVectorStore
            ...
            embedded_options = kwargs.get("embedded_options")
            additional_config = kwargs.get("additional_config")
            class_name = kwargs.get("class_name", "byte")
            class_schema = kwargs.get("class_schema")
            ...
            from byte.manager.vector_data.distributed import DistributedVectorStore

            stores = kwargs.get("stores")
            if stores is None:
                shard_specs = kwargs.get("shards", []) or []
                stores = []
                for shard_spec in shard_specs:
                    shard_payload = dict(shard_spec or {})
                    shard_name = shard_payload.pop("name", "")
                    if not shard_name:
                        raise ParamError("Each distributed vector shard requires a `name`.")
                    stores.append(VectorBase.get(shard_name, **shard_payload))
            vector_base = DistributedVectorStore(
                stores=stores,
                top_k=top_k,
                shard_strategy=kwargs.get("shard_strategy", "hash"),
            )
        else:
