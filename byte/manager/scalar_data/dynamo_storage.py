...
from byte.manager.scalar_data.base import Answer, CacheData, CacheStorage, Question, QuestionDep
from byte.utils import load_optional_module
from byte.utils.log import byte_log


def _load_boto3_dynamo_stack() -> tuple[Any, ...]:
    conditions = load_optional_module("boto3.dynamodb.conditions", package="boto3")
    types_module = load_optional_module("boto3.dynamodb.types", package="boto3")
    session_module = load_optional_module("boto3.session", package="boto3")
    return conditions, types_module, session_module
...
class DynamoStorage(CacheStorage):
    ...
    def __init__(
        ...
        conditions, types_module, session_module = _load_boto3_dynamo_stack()
        self._dynamo_attr = conditions.Attr
        self._dynamo_key = conditions.Key
        self._dynamo_binary = types_module.Binary
        self._aws_session = session_module.Session(
    ...
    def create(self) -> None:
        ...
        if not self._does_table_already_exist_and_is_active("byte_questions"):
            self._dynamo.create_table(
                TableName="byte_questions",
                KeySchema=[
        ...
        if not self._does_table_already_exist_and_is_active("byte_reports"):
            self._dynamo.create_table(
                TableName="byte_reports",
                KeySchema=[
    ...
    def batch_insert(self, all_data: list[CacheData]) -> list[str]:
        ...
        """
        table = self._dynamo.Table("byte_questions")
        ...
        with table.batch_writer() as batch:
            for item in all_data:
                item_deps = (
                    item.question.deps
                    if isinstance(item.question, Question) and item.question.deps is not None
                    else []
                )
                    ...
                    Item=self._strip_all_props_with_none_values(
                        {
                            ...
                            "last_access": item.last_access.isoformat(timespec="microseconds")
                            if item.last_access is not None
                            else None,
                            "embedding_data": self._dynamo_binary(item.embedding_data.tobytes())
                            if item.embedding_data is not None
                            else None,
    ...
    def get_data_by_id(self, key: str) -> CacheData | None:
        table = self._dynamo.Table("byte_questions")
        table.wait_until_exists()
        ...
        sessions_containing_question = table.query(
            KeyConditionExpression=self._dynamo_key("pk").eq(key_with_prefix)
            & self._dynamo_key("id").begins_with("sessions#"),
        )["Items"]
        ...
        cache_data.session_id = [
            session["id"].split("#")[1] for session in sessions_containing_question
        ]
        return cache_data

    def mark_deleted(self, keys: str) -> None:
        table = self._dynamo.Table("byte_questions")
        table.wait_until_exists()
    ...
    def clear_deleted_data(self) -> None:
            ...
            for i in range(1, self.max_cardinality_suffix + 1):
                futures.append(
                    executor.submit(
                        table.query,
                        IndexName="gsi_questions_by_deletion_status",
                        KeyConditionExpression=self._dynamo_key("deleted").eq(f"True_{i}"),
                    )
                )
        ...
        soft_deleted_entries_per_partition = [
            response.result()["Items"] for response in completed_responses
        ]
    ...
    def get_ids(self, deleted: bool = True) -> list[str]:
        ...
        def run_scan_operation(last_evaluated_key) -> Any:
            filter_expression = self._dynamo_attr("id").begins_with("questions#") & self._dynamo_attr(
                "deleted"
            ).begins_with(f"{deleted}_")
    ...
    def count(self, state: int = 0, is_all: bool = False) -> int:
        table = self._dynamo.Table("byte_questions")
        table.wait_until_exists()
        key_condition_expression = self._dynamo_attr("id").begins_with("questions#")

        if not is_all and state == 0:
            key_condition_expression &= self._dynamo_attr("deleted").begins_with("False_")
        elif not is_all and state != 0:
            key_condition_expression &= self._dynamo_attr("deleted").begins_with("True_")

        # DynamoDB Scan with Select="COUNT" still respects the 1 MB-per-page scan limit
        # and returns LastEvaluatedKey when the scan spans multiple pages, so pagination is required.
        def run_scan_operation(last_evaluated_key) -> Any:
            if last_evaluated_key is None:
                return table.scan(
                    IndexName="gsi_questions_by_deletion_status",
    ...
    def add_session(self, question_id: str, session_id: str, session_question: str) -> None:
        table = self._dynamo.Table("byte_questions")
        table.wait_until_exists()
    ...
    def list_sessions(self, session_id=None, key=None) -> list[str]:
        ...
        if session_id and key:
            response = table.query(
                ...
                KeyConditionExpression=self._dynamo_key("id").eq(f"sessions#{session_id}"),
                FilterExpression=self._dynamo_attr("pk").eq(f"questions#{key}"),
            )
        elif session_id:
            response = table.query(
                IndexName="gsi_items_by_type",
            ...
            response = table.query(
                ProjectionExpression="id",
                KeyConditionExpression=self._dynamo_key("pk").eq(f"questions#{key}")
                & self._dynamo_key("id").begins_with("sessions#"),
            )
        else:

            def run_scan_operation(last_evaluated_key) -> Any:
                filter_expression = self._dynamo_attr("id").begins_with("sessions#")
            ...
            for key in keys:
                futures.append(
                    executor.submit(
                        ...
                        KeyConditionExpression=self._dynamo_key("id").eq(f"sessions#{key}"),
                    )
                )
    ...
    def report_cache(
        ...
        table = self._dynamo.Table("byte_reports")
        table.wait_until_exists()
    ...
    def _does_table_already_exist_and_is_active(self, table_name: str) -> bool:
        try:
            assert self._dynamo.Table(table_name).table_status in ["ACTIVE", "UPDATING"]
        except self._dynamo.meta.client.exceptions.ResourceNotFoundException:
            return False
        return True

    def _wait_until_table_exists(self, table_name: str) -> None:
        """
    ...
    def _response_item_to_cache_data(self, question_resp: dict) -> CacheData | None:
        ...
        return CacheData(
            ...
            last_access=datetime.fromisoformat(question_resp["last_access"])
            if "last_access" in question_resp
            else None,
        )
    ...
    def _serialize_deleted_value(
        self, value: bool, suffix_value=None
    ) -> str:
        """
        if suffix_value is None:
            suffix_value = randint(1, max_cardinality_suffix)
