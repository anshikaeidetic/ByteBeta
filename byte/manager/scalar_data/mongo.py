...
def _load_mongoengine() -> Any:
    return load_optional_module("mongoengine", package="mongoengine")


def get_models() -> tuple[Any, ...]:
    mongoengine = _load_mongoengine()
    fields = mongoengine.fields
...
class MongoStorage(CacheStorage):
    ...
    By default, data is stored 'byte' database and following collections are created to store the data
        1. 'sessions'
        2. 'answers'
        3. 'questions'
        4. 'question_deps'
    ...
    :type host: int
    :param dbname: database name, default value 'byte'
    :type host: str
    :param : Mongo database name, default value 'byte'
    :type host: str
    ...
    def __init__(
        ...
        **kwargs,
        ...
        mongoengine = _load_mongoengine()
        self.con = mongoengine.connect(
            host=host, port=port, db=dbname, username=username, password=password, **kwargs
        )
        (
            self._ques,
            self._answer,
    ...
    def _insert(self, data: CacheData) -> Any:
    ...
    def get_data_by_id(self, key) -> CacheData | None:
    ...
    def get_ids(self, deleted: bool = True) -> Any:
