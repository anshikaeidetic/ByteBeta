...
from byte.utils import import_sql_client
from byte.utils.error import NotFoundError

SQL_URL = {
    "sqlite": "sqlite:///./sqlite.db",
    "duckdb": "duckdb:///./duck.db",
    "postgresql": "postgresql+psycopg2://postgres@127.0.0.1:5432/postgres",
    "mysql": "mysql+pymysql://root@127.0.0.1:3306/mysql",
    "mariadb": "mariadb+pymysql://root@127.0.0.1:3307/mysql",
    "sqlserver": "mssql+pyodbc://sa@127.0.0.1:1434/msdb?driver=ODBC+Driver+17+for+SQL+Server",
    "oracle": "oracle+cx_oracle://oracle@127.0.0.1:1521/?service_name=helowin&encoding=UTF-8&nencoding=UTF-8",
}
TABLE_NAME = "byte"
...
class CacheBase:
    ...
    :type name: str
    :param sql_url: the url of the sql database for cache, such as '<db_type>+<db_driver>://<user>@<host>:<port>/<database>',
        and the default value is related to the `cache_store` parameter,

        - 'sqlite:///./sqlite.db' for 'sqlite',
        - 'duckdb:///./duck.db' for 'duckdb',
        - 'postgresql+psycopg2://postgres@127.0.0.1:5432/postgres' for 'postgresql',
        - 'mysql+pymysql://root@127.0.0.1:3306/mysql' for 'mysql',
        - 'mariadb+pymysql://root@127.0.0.1:3307/mysql' for 'mariadb',
        - 'mssql+pyodbc://sa@127.0.0.1:1434/msdb?driver=ODBC+Driver+17+for+SQL+Server' for 'sqlserver',
        - 'oracle+cx_oracle://oracle@127.0.0.1:1521/?service_name=helowin&encoding=UTF-8&nencoding=UTF-8' for 'oracle'.
    :type sql_url: str
    :param table_name: the table name for sql database, defaults to 'byte'.
    :type table_name: str
    ...
    Example:
        .. code-block:: python
            ...
            from byte.manager import CacheBase
    ...
    def get(name, **kwargs) -> Any:
        if name in [
            ...
            from byte.manager.scalar_data.sql_storage import SQLStorage
            ...
            from byte.manager.scalar_data.mongo import MongoStorage
            ...
            from byte.manager.scalar_data.redis_storage import RedisCacheStorage
