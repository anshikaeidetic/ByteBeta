...
from byte.manager.data_manager import MapDataManager, SSDataManager
...
def manager_factory(
    ...
    get_data_container: Callable | None = None,
    scalar_params=None,
    vector_params=None,
    object_params=None,
    ...
    if len(db_infos) not in [2, 3]:
        raise RuntimeError(
            f'Error manager format: {manager}, the correct is "{{scalar}},{{vector}},{{object}}", object is optional'
        )
    ...
    if scalar == "redis" and eviction_manager == "redis":
        ...
        byte_log.info(
            "Since Scalar Storage and Eviction manager are both redis, "
        ...
        s.init_eviction_params(
            ...
            maxmemory_samples=eviction_params.get(
                "maxmemory_samples", scalar_params.get("maxmemory_samples")
            ),
        )
    ...
    if eviction_manager == "memory":
        return get_data_manager(
            ...
            eviction_params.get("clean_size"),
            eviction_params.get("eviction", "LRU"),
        )

    e = EvictionBase(name=eviction_manager, **eviction_params)
    return get_data_manager(s, v, o, e)


def get_data_manager(
    cache_base: CacheBase | str | None = None,
    vector_base: VectorBase | str | None = None,
    object_base: ObjectBase | str | None = None,
    eviction_base: EvictionBase | str | None = None,
    max_size: int = 1000,
    clean_size=None,
    eviction: str = "LRU",
    ...
    Example:
        .. code-block:: python
            ...
            from byte.manager import get_data_manager, CacheBase, VectorBase
    ...
    if not cache_base or not vector_base:
        raise ValueError("Both cache_base and vector_base must be provided for SSDataManager.")
    return SSDataManager(
        cache_base, vector_base, object_base, eviction_base, max_size, clean_size, eviction
    )
