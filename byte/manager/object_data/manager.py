...
class ObjectBase:
    ...
    def get(name, **kwargs) -> Any:
        if name == "local":
            from byte.manager.object_data.local_storage import (
                LocalObjectStorage,  # pylint: disable=import-outside-toplevel
            )

            object_base = LocalObjectStorage(kwargs.get("path", "./local_obj"))
        elif name == "s3":
            from byte.manager.object_data.s3_storage import (
                S3Storage,  # pylint: disable=import-outside-toplevel
            )

            object_base = S3Storage(
                kwargs.get("bucket"),
