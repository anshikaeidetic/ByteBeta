...
def euclidean_distance_calculate(vec_l: np.array, vec_r: np.array) -> Any:
    return np.sum((vec_l - vec_r) ** 2)


def reweight(weights, length) -> Any:
...
class SequenceMatchEvaluation(SimilarityEvaluation):
    ...
    :type embedding_extractor: byte.embedding.base.BaseEmbedding
    ...
    def __init__(self, weights: list[float], embedding_extractor: str, embedding_config=None) -> None:
