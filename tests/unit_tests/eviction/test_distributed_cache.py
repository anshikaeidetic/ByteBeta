...
redis_om = pytest.importorskip("redis_om")
RedisConnectionError = redis_exceptions.ConnectionError
get_redis_connection = redis_om.get_redis_connection

from byte.embedding import Onnx
from byte.manager import CacheBase, VectorBase, get_data_manager, manager_factory
...
class TestDistributedCache(unittest.TestCase):
    ...
    def setUp(cls) -> None:
        try:
            ...
            raise unittest.SkipTest(f"Redis is not available for this test suite: {exc}") from exc
    ...
    def test_redis_only_cache_eviction(self) -> None:
        manager = manager_factory(
            "redis,faiss",
            eviction_manager="redis",
            vector_params={"dimension": 5},
            eviction_params=dict(url=self.url, maxmemory="100mb", policy="allkeys-lru"),
        )
    ...
    def test_eviction_base_str(self) -> None:
    ...
    def test_redis_sqlite_cache_eviction(self) -> None:
    ...
    def test_noeviction_policy(self) -> None:
        onnx = Onnx()
        data_manager = manager_factory(
            ...
            eviction_params={"maxmemory": "0", "policy": "noeviction"},
        )
        questions = []
        answers = []
        idx_list = []
        ...
        for _idx, question, answer in zip(idx_list, questions, answers):
            embedding = onnx.to_embeddings(question)
            data_manager.save(question=question, answer=answer, embedding_data=embedding)
    ...
    def test_ttl(self) -> None:
        ...
        data_manager = manager_factory(
            ...
            scalar_params={"url": self.url},
            vector_params={"dimension": onnx.dimension},
            eviction_params={"maxmemory": "0", "policy": "noeviction", "ttl": 1},
        )
        questions = []
        answers = []
        embeddings = []
    ...
    def test_redis_only_config(self) -> None:
        onnx = Onnx()
        data_manager = get_data_manager(
            cache_base=CacheBase("redis", maxmemory="100mb", policy="allkeys-lru"),
    ...
    def test_redis_only_with_no_op_eviction_config(self) -> None:
        onnx = Onnx()
        data_manager = get_data_manager(
            cache_base=CacheBase("redis", maxmemory="100mb", policy="allkeys-lru"),
            vector_base=VectorBase("faiss", dimension=onnx.dimension),
