...
from byte.adapter.api import _get_pre_context_function


def test_summarization_context_process() -> None:
    try:
        context_process = _get_pre_context_function(
            "summarization",
            kws={"model_name": "facebook/bart-large-cnn", "target_length": 512},
        )
        tokenizer = RobertaTokenizer.from_pretrained("roberta-base")
    except Exception as exc:  # pragma: no cover - environment dependent
        pytest.skip(f"Summarization context is not available for this test suite: {exc}")
