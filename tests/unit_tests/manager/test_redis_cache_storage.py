...
from byte.manager.scalar_data.base import CacheData, Question
from byte.manager.scalar_data.redis_storage import RedisCacheStorage
from byte.utils import import_redis
...
class TestRedisStorage(unittest.TestCase):
    ...
    def test_normal(self) -> None:
        redis_storage = RedisCacheStorage(global_key_prefix=self.test_dbname, url=self.url)
        data = []
        for i in range(1, 10):
            data.append(
                CacheData(
    ...
    def test_cache_configuration(self) -> None:
        redis_storage = RedisCacheStorage(
            global_key_prefix=self.test_dbname,
            url=self.url,
            maxmemory_samples=5,
            maxmemory="4mb",
            policy="allkeys-lru",
