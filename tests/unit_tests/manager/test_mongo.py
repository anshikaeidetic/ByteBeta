...
def test_mongo() -> None:
    ...
    try:
        ...
        pytest.skip(f"MongoDB is not available for this test suite: {exc}")


def _clear_test_db(dbname) -> None:
    con = connect(db=dbname)
    con.drop_database(dbname)
    disconnect()


def _inner_test_normal(dbname: str) -> None:
...
def _inner_test_with_deps(dbname: str) -> None:
    mongo_storage = MongoStorage(dbname=dbname)
...
def _test_create_on(dbname) -> None:
    mongo_storage = MongoStorage(dbname=dbname)
