...
from byte.manager import CacheBase, ObjectBase, VectorBase
...
class TestFactory(unittest.TestCase):
    def test_normal(self) -> None:
        ...
        with mock.patch("byte.manager.vector_data.milvus.Milvus.__init__") as mock_init:
            mock_init.return_value = None
            self.assertIsNotNone(
                manager_factory("sqlite,milvus", vector_params={"dimension": 5, "port": "9999"})
            )
    ...
    def test_manager(self) -> None:
        with TemporaryDirectory() as root:
            ...
            m.save(
                "test_question", Answer(b"my test data", DataType.IMAGE_BASE64), np.random.rand(5)
            )
            ...
            m.save(
                "test_question2",
                [
                    Answer(b"question2_BASE64", DataType.IMAGE_BASE64),
                    Answer("question2_STR", DataType.STR),
                ],
                emb,
            )
    ...
    def test_manager_factory(self) -> None:
        with TemporaryDirectory() as root:
            data_dir = os.path.join(root, "dir_not_exist")
            m = manager_factory(
                "sqlite,faiss,local", data_dir=data_dir, vector_params={"dimension": 5}
            )
            m.save(
                "test_question", Answer(b"my test data", DataType.IMAGE_BASE64), np.random.rand(5)
            )
