"""Dynamo scalar storage tests for persistence, filters, and search semantics."""
...
skip_module_if_feature_missing("dynamo")

from byte.manager.scalar_data.base import CacheData, DataType, Question, QuestionDep
from byte.manager.scalar_data.dynamo_storage import DynamoStorage
from byte.utils import import_boto3

import_boto3()

from boto3 import client as awsclient
from boto3 import resource as awsresource
...
class TestDynamoCacheStorage(unittest.TestCase):
    ...
    _boto_config = Config(connect_timeout=1, read_timeout=1, retries={"max_attempts": 0})

    @classmethod
    def setUpClass(cls) -> None:
        try:
            cls._dynamo_client().list_tables()
        except BotoCoreError as exc:
            raise unittest.SkipTest(f"DynamoDB Local is not available for this test suite: {exc}") from exc
    ...
    def test_create_is_idempotent(self) -> None:
        ...
        table = self._dynamo_resource().Table("byte_questions")
        assert table is not None

        table = self._dynamo_resource().Table("byte_reports")
        assert table is not None

    def test_creation_and_batch_insert(self) -> None:
        ...
        for item in items:
            original_data_inserted = next(
                (
                    cache_data
                    for cache_data in cache_entries_to_insert
                    if (
                        cache_data.question == item["question"]
                        or (isinstance(cache_data.question, Question)
                        and cache_data.question.content == item["question"])
                    )
                ),
                None,
            )
            ...
            if item["id"].startswith("sessions#"):
                ...
                assert (
                    datetime.fromisoformat(item["create_on"]) > time_before_insertion
                    and datetime.fromisoformat(item["create_on"]) < datetime.utcnow()
                )
                ...
                assert (
                    original_data_inserted.embedding_data
                    == np.frombuffer(item["embedding_data"].value, dtype=np.float32)
                ).all()

                # if the question has dependencies, make sure they're all there
                if (
                    isinstance(original_data_inserted.question, Question)
                    and original_data_inserted.question.deps is not None
                ):
                ...
                for answer in original_data_inserted.answers:
                    matching_cache_entry = next(
                        (
                            answer_dict
                            for answer_dict in item["answers"]
                            if answer.answer == answer_dict["answer"]
                        ),
                        None,
                    )
                    assert matching_cache_entry is not None
    ...
    def test_clear_deleted_data(self) -> None:
    ...
    def test_add_session(self) -> None:
        ...
        self.dynamo_cache_storage.add_session(
            persisted_id, session_id1, item_to_insert.question.content
        )
        self.dynamo_cache_storage.add_session(
            persisted_id, session_id2, item_to_insert.question.content
        )
    ...
    def test_list_sessions(self) -> object:
        items_to_insert = [
        ...
        self.dynamo_cache_storage.add_session(
            persisted_ids[0], session_id1, items_to_insert[0].question.content
        )
        self.dynamo_cache_storage.add_session(
            persisted_ids[0], session_id2, items_to_insert[0].question.content
        )
        self.dynamo_cache_storage.add_session(
            persisted_ids[1], session_id3, items_to_insert[1].question.content
        )

        def ids_without_prefix(ids) -> object:
            return [
                    id_with_prefix.replace("sessions#", "") for id_with_prefix in ids
                ]

        # now we should be able to see those sessions
        session_ids_with_prefix = self.dynamo_cache_storage.list_sessions()
        assert sorted(ids_without_prefix(session_ids_with_prefix)) == sorted(
        ...
        # now filter by both
        session_ids_with_prefix = self.dynamo_cache_storage.list_sessions(
            session_id=session_id2, key=persisted_ids[0]
        )
    ...
    def test_delete_session(self) -> None:
        ...
        self.dynamo_cache_storage.batch_insert(items_to_insert)
    ...
    def test_get_data_by_id(self) -> None:
        ...
        for answer in entry.answers:
            matching_answer = next(
                (
                    answer_to_insert
                    for answer_to_insert in item_to_insert.answers
                    if answer.answer == answer_to_insert.answer
                    and answer.answer_type == answer_to_insert.answer_type
                ),
                None,
            )
        ...
        assert entry_accessed_again.last_access > time_before_insertion

        entry_accessed_thrice = self.dynamo_cache_storage.get_data_by_id(persisted_id)
        assert entry_accessed_thrice.last_access > entry_accessed_again.last_access
        ...
        table.update_item(
            ...
            ExpressionAttributeValues={":deleted": "True_1"},
        )

        entry = self.dynamo_cache_storage.get_data_by_id(persisted_id)
        assert entry is None

    def test_report_cache(self) -> None:
        item_to_insert = {
            "user_question": "how many people in this picture?",
            "cache_question": "how many people are in this picture?",
            "cache_question_id": f"questions#{uuid4()!s}",
            "cache_answer": "5",
    ...
    def _random_cachedata_without_dependencies(self, session_id=None) -> object:
        question_id = uuid4()
        return CacheData(
    ...
    def _random_cachedata_with_dependencies(self, session_id=None) -> object:
        question_id = uuid4()
        return CacheData(
            question=Question(
                ...
                deps=[
                    QuestionDep(
                        name="text", data="how many people in this picture", dep_type=DataType.STR
                    ),
    ...
    def _dynamo_resource(self) -> object:
        return awsresource(
            "dynamodb",
    ...
    @classmethod
    def _dynamo_client(cls) -> object:
        return awsclient(
            "dynamodb",
            endpoint_url=cls._dynamodb_local_endpoint_url,
            aws_access_key_id="test",
            aws_secret_access_key="test",
