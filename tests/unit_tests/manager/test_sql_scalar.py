...
class TestSQLStore(unittest.TestCase):
    ...
    def test_duckdb(self) -> None:
        try:
            ...
            self.skipTest(f"DuckDB support is not available for this test suite: {exc}")
        self._inner_test_normal("duckdb")
        self._inner_test_with_deps("duckdb")

    def _inner_test_normal(self, db_name: str) -> None:
    ...
    def _inner_test_with_deps(self, db_name: str) -> None:
        with TemporaryDirectory() as root:
    ...
    def test_flush_satisfies_storage_contract(self) -> None:
        with TemporaryDirectory() as root:
            db_path = Path(root) / "sqlite_flush.db"
            db = SQLStorage(db_type="sqlite", url="sqlite:///" + str(db_path))
            db.create()
            db.flush()
