...
from byte.similarity_evaluation import NumpyNormEvaluation


def embedding_func(x) -> object:
    return np.array([1, 1])
...
def _test_evaluation(evaluation) -> None:
    ...
    score = evaluation.evaluation({"question": "test"}, {"question": "test"})
    assert math.isclose(score, 2.0), score

    score = evaluation.evaluation({"question": "test1"}, {"question": "test2"})
    assert math.isclose(score, 2.0), score
...
def test_get_eval() -> None:
    _get_eval(
        strategy="numpy", kws={"enable_normal": True, "question_embedding_function": embedding_func}
    )
